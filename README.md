...
Universe Evolution Lab 是一个命令行 MVP，用于创建、运行、观察、分支、导出和批量比较虚拟宇宙、复杂结构、生命、文明演化实验。当前重点是小规模、可读、可扩展的本地模拟，不引入复杂依赖。
...
- 模拟规则保持轻量，当前覆盖复杂结构、Population、物种和文明的基础演化。

## 支持模式

- `life_burst`：复杂性涌现/生命大爆发观察模式，初始生成无生命复杂结构，观察结构是否能长期演化为准生命、生命谱系、Population、物种和文明。
